...
(function () {
  ...
  function wireLocalCloseHandlers() {
    const close = () => CS.modal?.closeModal?.();
    sheetClose?.addEventListener('click', close);
    sheetModal?.addEventListener('click', (e) => { if (e.target === sheetModal) close(); });
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && sheetModal && !sheetModal.classList.contains('hidden')) close();
    });
  }
  // Wire once on module load
  try { wireLocalCloseHandlers(); } catch (e) { console.warn('[CharSheet] close wiring failed', e); }
...
function calcWeaponAttackBonus(sheet, weapon) {
  ...
  const prof = weapon.prof ? CS.utils.getProfBonus(sheet) : 0;
...
function updateWeaponsBonuses(root, sheet) {
  ...
  cards.forEach(card => {
    ...
    if (profDot) {
      ...
      profDot.title = `Владение: +${CS.utils.getProfBonus(sheet)} к бонусу атаки`;
    }
...
function rerenderCombatTabInPlace(root, player, canEdit) {
  ...
  CS.bindings.bindSkillBoostDots(root, player, canEdit);
  CS.bindings.bindAbilityAndSkillEditors(root, player, canEdit);
...
function bindCombatEditors(root, player, canEdit) {
  ...
  function renderActiveTab(tabId, vm, canEdit) {
    ...
    if (tabId === "basic") {
      const fn = pick("CS.tabs.renderBasicTab");
      if (fn) return fn(vm, canEdit);
      return `<div class="sheet-note">Ошибка: не загружена вкладка «Основа» (CS.tabs.renderBasicTab).</div>`;
    }

    if (tabId === "spells") {
      const fn = (spells && typeof spells.renderSpellsTab === "function") ? spells.renderSpellsTab
        : pick("CS.tabs.renderSpellsTab");
      if (fn) return fn(vm, canEdit);
      return `<div class="sheet-note">Ошибка: не загружена вкладка «Заклинания».</div>`;
    }

    if (tabId === "combat") {
      const fn = pick("CS.tabs.renderCombatTab");
      if (fn) return fn(vm, canEdit);
      return `<div class="sheet-note">Ошибка: не загружена вкладка «Бой» (CS.tabs.renderCombatTab).</div>`;
    }

    if (tabId === "inventory") {
      const fn = (tabs && typeof tabs.renderInventoryTab === "function") ? tabs.renderInventoryTab
        : pick("CS.tabs.renderInventoryTab");
      if (fn) return fn(vm, canEdit);
  ...
  function renderSheetModal(player, opts = {}) {
    ...
    CS.bindings.restoreUiStateToDom(player);
    ...
    CS.bindings.bindSkillBoostDots(sheetContent, player, canEdit);
    CS.bindings.bindSaveProfDots(sheetContent, player, canEdit);
    CS.bindings.bindStatRollButtons(sheetContent, player);
    CS.bindings.bindAbilityAndSkillEditors(sheetContent, player, canEdit);
    ...
    CS.db.wireQuickBasicInteractions(sheetContent);
  ...
  function init(context) {
    ...
    try {
      ...
      console.warn('[CharSheet] CS.db.ensureWiredCloseHandlers failed', e);
    }
  }
